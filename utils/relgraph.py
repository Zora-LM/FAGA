...
def generate_relation_graphs(g, num_rel, B): #(triplet, num_ent, num_rel, B):
    row, col = g.edges()
    rel = g.edata['type']
    num_ent = g.num_nodes()

    triplet = [[h, r, t] for h, t, r in zip(row, col, rel)]
    ...
    # rel_graph = dgl.graph((relation_triplets[:, 0], relation_triplets[:, 1]), num_nodes=num_rel)
    # rel_graph.ndata['feat'] = torch.tensor(np.arange(num_rel))
    # rel_graph.edata['bin'] = torch.tensor(relation_triplets[:, 2])
