...
class Finetuner():
    def __init__(self, args, graph_classifier, train, valid_evaluator=None, collate_fn=None):
        ...
        if args.optimizer == "Adam":
            self.optimizer = optim.Adam(model_params, lr=args.finetune_lr, weight_decay=self.args.l2)
    ...
    def train_epoch(self):
        ...
        for b_idx, batch in enumerate(dataloader):
            ...
            score_pos = self.graph_classifier.forward_eval(data_pos, rel_graph)
            score_neg = self.graph_classifier.forward_eval(data_neg, rel_graph)
            # Losses
