...
import pickle
import dgl
from utils.graph_utils import ssp_multigraph_to_dgl, incidence_matrix, ssp_multigraph_to_dgl_inv_edge
from utils.data_utils import process_files, process_files_, save_to_file, plot_rel_dist, data2pkl, data2pkl_new_rel
...
def generate_subgraph_datasets(params, splits=['train', 'valid'], saved_relation2id=None, max_label_value=None):
    ...
    if not os.path.exists(f'{params.main_dir}/data/{data_name}.pkl'):
        data2pkl_new_rel(params.main_dir, data_name)
    ...
    adj_list, triplets, id2entity, id2relation = process_files_(params.file_paths, entity2id, relation2id)
    ...
    for split_name, split in graphs.items():
        ...
        split['pos'], split['neg'] = sample_neg(adj_list, split['triplets'], params.num_neg_samples_per_link,
                                                max_size=split['max_size'], constrained_neg_prob=params.constrained_neg_prob)

    if testing:
        directory = params.exp_dir # os.path.join(params.main_dir, 'data/{}/'.format(params.dataset))
        save_to_file(directory, f'neg_{params.step}_{params.test_file}_{params.constrained_neg_prob}.txt',
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, args, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None,
        ...
        data_name = args.dataset.replace('_ind', '')
        data = pickle.load(open(f'{args.main_dir}/data/{data_name}.pkl', 'rb'))
        relation2id = data['relation2id']
        if 'ind' not in args.dataset:
            entity2id = data['train_graph']['entity2id']
        else:
            entity2id = data['ind_test_graph']['entity2id']

        ssp_graph, _, id2entity, id2relation = process_files_(args.file_paths, entity2id, relation2id)
        ...
        if self.args.add_reverse_rels:
            self.graph = ssp_multigraph_to_dgl_inv_edge(ssp_graph)
            self.num_rels = len(ssp_graph)*2
            self.aug_num_rels = len(ssp_graph)*2
        else:
        ...
        rel_graph = generate_relation_graphs(self.graph, num_rel=self.aug_num_rels, B=self.args.num_bin)
        self.rel_graph = torch.tensor(rel_graph)
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        subgraph = self.graph.subgraph(nodes) #dgl.DGLGraph(self.graph.subgraph(nodes)) #
