...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    BYTES_PER_DATUM = get_average_subgraph_size(100, list(graphs.values())[0]['pos'], A, params) * 3
    links_length = 0
    for split_name, split in graphs.items():
        links_length += (len(split['pos']) + len(split['neg'])) * 2
    map_size = int(links_length * BYTES_PER_DATUM)

    env = lmdb.open(params.db_path, map_size=map_size, max_dbs=6) # create lmdb environment

    def extraction_helper(A, links, g_labels, split_env):

        with env.begin(write=True, db=split_env) as txn:
            txn.put('num_graphs'.encode(), (len(links)).to_bytes(int.bit_length(len(links)), byteorder='little')) # .put(key, value, dupdata=True, overwrite=True, append=False, db=None)
    ...
    for split_name, split in graphs.items():
        ...
        split_env = env.open_db(db_name_pos.encode()) # .open_db() creates new named databases
...
def node_label(subgraph, max_distance=1):
    ...
    dist_to_roots = [np.clip(ssp.csgraph.dijkstra(sg, indices=[0], directed=False, unweighted=True, limit=1e6)[:, 1:],
                             0, 1e7) for r, sg in enumerate(sgs_single_root)]
