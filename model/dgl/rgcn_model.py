...
class RGCN(nn.Module):
    def __init__(self, params):
        ...
        self.num_hidden_layers = params.num_layer_ent  #params.num_gcn_layers
    ...
    def build_model(self):
        ...
        # i2h
        if self.args.is_input_layer:
            i2h = self.build_input_layer()
        else:
            i2h = self.build_hidden_layer(idx=0)
        if i2h is not None:
...
class RGCN2(nn.Module):
    ...
    def forward(self, g, rel_emb=None):
        for layer in self.layers:
            layer(g, rel_emb)
        return g.ndata.pop('h')
